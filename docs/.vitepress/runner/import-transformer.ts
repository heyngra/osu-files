export function transformExample(source: string): { code: string; names: string[] } {
  ...
  const names: string[] = []
  let code = source.replace(/import\s*\{([^}]+)\}\s*from\s*['"]osu-files['"]\s*;?/g, (_match, imports: string) => {
  ...
  if (/\b(?:window|document|parent|localStorage|sessionStorage)\b/.test(code)) throw new Error('Page and storage globals are unavailable in the runner.')
