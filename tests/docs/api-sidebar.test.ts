...
const manifest = JSON.parse(readFileSync(resolve(root, 'docs/.generated/api-manifest.json'), 'utf8')) as Array<{ documentationPage: string }>
...
describe('generated API sidebar', () => {
  it('exposes every top-level declaration', () => {
    const topLevel = manifest.filter(item => !item.documentationPage.includes('#'))
